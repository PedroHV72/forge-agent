...
**Budget rule (normative).** The source pointer is charged against the same budget it protects — it is never an unbudgeted addition on top of `budgetChars`. Both truncators reserve space for the marker text before deciding how much content to keep, and both degrade in the same order when the full marker would not fit: **full marker with pointer → shorter marker without pointer → silent cut** (no marker at all only in the pre-existing legacy path where `opts.source` was never passed). The rendered result never exceeds `budgetChars`/`maxChars`.
