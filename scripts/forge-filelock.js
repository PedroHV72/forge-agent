...
function lockPathFor(cwd, filePath) { validatePath(filePath); return path.join(locksDir(cwd), `${encodePath(filePath)}.json`); }
...
function publicHolder(existing, now) { return { run_id: existing.run_id, session_id: existing.session_id, file_path: existing.file_path, acquired_at: existing.acquired_at, age_ms: ageOf(existing, now), generation: existing.generation }; }

function acquireFileLock(cwd, filePath, runId, sessionId, opts) {
  opts = opts || {}; validatePath(filePath); const ttlMs = positive(opts.ttlMs, DEFAULT_TTL_MS); const now = nowOf(opts); const file = lockPathFor(cwd, filePath);
  fs.mkdirSync(path.dirname(file), { recursive: true });
  return withGuard(cwd, filePath, () => {
    ...
    if (existing) {
      const age = ageOf(existing, now); const stale = age !== null && age > ttlMs;
    ...
    const meta = { run_id: runId || null, session_id: sessionId || null, file_path: String(filePath), intent: opts.intent || 'edit', generation: newToken(opts), owner_token: newToken(opts), acquired_at: now, renewed_at: now, ttl_ms: ttlMs };
...
function releaseFileLock(cwd, filePath, runId, ownerToken, generation) {
  ...
  const file = lockPathFor(cwd, filePath);
  const result = withGuard(cwd, filePath, () => { const existing = readLock(file); if (!existing) return { ok: false, reason: 'already_released' }; if (existing.owner_token !== ownerToken || existing.generation !== generation) return { ok: false, reason: 'owner_mismatch' }; const quarantine = `${file}.release-${generation}-${crypto.randomUUID()}`; try { fs.renameSync(file, quarantine); fs.unlinkSync(quarantine); return { ok: true, reason: 'released' }; } catch { return { ok: false, reason: 'already_released' }; } });
  return result.ok;
}
function checkFileLock(cwd, filePath, opts) { const existing = readLock(lockPathFor(cwd, filePath)); if (!existing) return { held: false }; return { held: true, holder: existing, age_ms: ageOf(existing, nowOf(opts || {})) }; }
