...
function parseArgs(argv = process.argv.slice(2)) { const out = { runtime: 'both', repo: path.resolve(__dirname, '..') }; for (let i = 0; i < argv.length; i++) { const arg = argv[i]; if (arg === '--runtime') out.runtime = argv[++i] || ''; else if (arg === '--repo') out.repo = path.resolve(argv[++i] || ''); else if (arg === '--claude-home') out.claudeHome = argv[++i]; else if (arg === '--codex-home') out.codexHome = argv[++i]; else if (arg === '--forge-home') out.forgeHome = argv[++i]; else if (arg === '--project-root') out.projectRoot = argv[++i]; else if (arg === '--dry-run') out.dryRun = true; else if (arg === '--update') out.update = true; else if (arg === '--json') out.json = true; else if (arg === '--help' || arg === '-h') out.help = true; else throw new Error(`opção desconhecida: ${arg}`); } if (!RUNTIMES.includes(out.runtime)) throw new Error(`runtime inválido: ${out.runtime}`); return out; }
function generate(options = {}) { const runtime = options.runtime || 'both'; if (!RUNTIMES.includes(runtime)) throw new Error(`runtime inválido: ${runtime}`); const reports = {}; for (const host of selected(runtime)) { const input = { ...options }; if (host === 'claude') reports.claude = claude.write(input); else reports.codex = codex.write(input); } return { runtime, selected: selected(runtime), reports, changed: Object.values(reports).some((report) => report.changed), dry_run: Boolean(options.dryRun) }; }
