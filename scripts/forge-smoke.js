...
// ── Section 92: routing domains reach the planner through rendered prompts ──
function smokeRoutingDomainsRendered() {
  process.stdout.write('\n▸ Section 92: rendered routing/repository planner headers\n');
  ...
  try {
    ...
    const mainBody = fs.readFileSync(__filename, 'utf8').slice(fs.readFileSync(__filename, 'utf8').lastIndexOf('async function main()'));
    assert(/smokeRoutingDomainsRendered\(\);/.test(mainBody), '(g) Section 92 is registered in main()');
  } finally {
    if (oldHome === undefined) delete process.env.HOME; else process.env.HOME = oldHome;
    if (oldUserProfile === undefined) delete process.env.USERPROFILE; else process.env.USERPROFILE = oldUserProfile;
    cleanup(dir);
  }
  pass('(final) Section 92: real rendered planner prompts carry routing and repository values with a biting fixture guard');
}
