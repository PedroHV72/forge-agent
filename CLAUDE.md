...
- **Milestone ativo:** — (idle). `master` @ `c729cc4` = v3.4.0 + **TASK-021** (detector de rota inerte), com a entrada `## Unreleased` no CHANGELOG aguardando o próximo corte de versão.
- **Suítes (medidas no master em 2026-08-01):** `run-tests.js` **57/57 suites** (34s) · `forge-smoke.js` **2009 passed, 0 failed, 0 skipped** (113s). Os `0 skipped` só acontecem com `svn` presente na máquina — em máquina sem svn as seções SVN pulam (e falham no CI Linux, por desenho).
- **Últimos releases:** **v3.2.0** trouxe **M017** (Suporte a SVN no sidecar multi-LLM, Fase 1 — 6 slices, o sidecar codex roda ponta a ponta numa working copy SVN sem tocar git, com a propriedade de segurança provada em fixture real) e **TASK-020** (fim da aceitação silenciosa de alegações `environment` falsas — ver a decisão de arquitetura homônima acima). **v3.3.0** e **v3.4.0** são do app macOS (`install.sh --with-app`), por três tasks soltas: atualização in-app com barra de progresso streaming, afordance de reinstalar, e a sidebar enxuta com a versão em execução no rodapé.
- **Próxima ação:** `/forge-new-milestone` para os **gates do harness ruidosos ou inertes** — brief em `.gsd/KNOWLEDGE-harness-gaps.md` com evidência de dois operadores, **13 unidades em 3 lotes** e 4 reenquadramentos (ler primeiro). A unidade **K** já saiu como TASK-021; restam do item 8 as unidades **J** (atribuir por `repo.worktree` no `forge-code-dir.js` + `forge-planner` emitindo `repo:` incondicionalmente — a causa raiz do caso lookchina), **L** (contador de repetição por `reason`) e **M** (a regra de nunca atribuir a uma recusa causa que a ferramenta não declarou). Alternativas: Fase 2 do SVN (isolação real — worktree/branch equivalente), ou os follow-ups em `.gsd/KNOWLEDGE.md § Review follow-ups`.
- **Papercut aberto:** os instaladores criam um `~/.claude/forge-agent-backup-<ts>/` por `--update` e nunca podam (chegou a 57 dirs / 26 MB antes da limpeza manual de 2026-07-30). Candidato ao lote 1 do brief acima — arquivo único, `install.sh` + `install.ps1`.
