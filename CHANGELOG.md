## v3.3.0 — The version you are running, and an installer you can watch

Three tasks about the same complaint: the app would not tell you where it stood. It
updated itself behind a silent spinner, it could only be made to run the installer when
a release happened to be pending, and it announced a version number that was the
repository's tag rather than the binary you had open.

### Added

- **The in-app installer shows what it is doing while it does it** (`T-20260729191241-atualizacao-barra`). "Atualizar" used to hand off to `install.sh` and then say nothing for minutes, so a slow update and a hung one looked identical and the only way to tell them apart was to give up. The installer now runs headless with its output streamed into the card: a progress bar, a phase label that is the answer on its own, and a log folded away because it is the appeal, not the answer — it is where a failed step explains itself. The precheck that refuses to update a dirty or diverged checkout now says that the refusal is **protection** and names the command that clears it, instead of reporting a failure the operator has to interpret. Relaunch is offered only on exit 0, and the relaunch sequence was reordered so that cancelling the live-session alert no longer leaves two copies of the app running.
