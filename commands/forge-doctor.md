...
Com `--fix`, execute `node scripts/forge-doctor.js --fix` apenas para os reparos reversíveis que o script declara. `--dry-run` não escreve. Para regenerar projeções, encaminhe `--regen-projection` diretamente ao script.
